...
extension XMLTests {
    ...
    func testExcessiveSpineItemsWithFoundation() throws {
        ...
        XCTAssertEqual(try xmlDocument.nodes(forXPath: "//package").first?.name, ridiXMLDocument[xPath: "//package"].first?.name)
        XCTAssertEqual(try xmlDocument.nodes(forXPath: "//*:title").first?.name, ridiXMLDocument[xPath: "//*:title"].first?.name)
    }
}
